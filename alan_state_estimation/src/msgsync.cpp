 ...
 * \brief classes for vision-based relative localization failsafe functions
 */
...
void alan::MsgSyncNodelet::set_all_sfc(Eigen::Translation3d t_current,Eigen::Quaterniond q_current)
{
    ...
    alan_visualization::Tangent temp_tangent = set_plane_bound(
        temp_normal,
        t_current.translation() + q_rotate_vector(q_current,Eigen::Vector3d(final_corridor_length - 0.01 ,0,0))
    );
