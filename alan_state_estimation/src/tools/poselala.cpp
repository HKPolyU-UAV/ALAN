...
int main( int argc, char* argv[])
{
  ...
  cv::solvePnP(objectPoints, imagePoints, cameraMatrix, distCoeffs, rvec, tvec);//, cv::SOLVEPNP_EPNP
  ...
  for(unsigned int i = 0; i < projectedPoints.size(); ++i)
    {
        ...
        std::cout<<cv::Point2d(imagePoints[i] - projectedPoints[i]).x<<std::endl;;
        std::cout<<cv::Point2d(imagePoints[i] - projectedPoints[i]).y<<std::endl;;              
    }
